...
#excel转换为txt
'''
df = pd.read_excel('Data/故障信息.xlsx', header=0)  # 使用pandas模块读取数据
print('开始写入txt文件...')
df1 = df.iloc[:, 5]
df1.to_csv('Data/故障信息.txt', header=None, sep=',', index=False)  # 写入，逗号分隔
'''

file_corpus = 'Hello/answer/bm25data/故障信息.txt'
stop_words = 'Hello/answer/bm25data/cn_stopwords.txt'
